...
export function OdsSelector() {
  const [selectedODSIds, setSelectedODSIds] = useState<string[]>([])
  ...
  const handleSelectODS = (odsId: string) => {
    setSelectedODSIds(prev => {
      const newSelection = prev.includes(odsId) 
        ? prev.filter(id => id !== odsId) 
        : [...prev, odsId]
      return newSelection.sort((a, b) => {
        const aNumber = odsList.find(ods => ods.id === a)?.number || 0
        const bNumber = odsList.find(ods => ods.id === b)?.number || 0
  ...
  const handleRemoveODS = (odsId: string, event: React.MouseEvent) => {
    ...
    setSelectedODSIds(prev => prev.filter(id => id !== odsId))
  }
            ...
            <SelectGroup>
              {filteredODS.map((ods) => (
                <SelectItem 
                  key={ods.id} 
                  value={ods.id} 
                  className="focus:bg-transparent focus:text-inherit hover:bg-gray-100"
                >
                  <div className="flex items-center">
                    <Checkbox
                      checked={selectedODSIds.includes(ods.id)}
                      onCheckedChange={() => handleSelectODS(ods.id)}
                      className="mr-2 border-2 data-[state=checked]:text-white"
                      style={{
                        borderColor: ods.color,
                        backgroundColor: selectedODSIds.includes(ods.id) ? ods.color : 'transparent'
                      }}
                    />
                    <div 
