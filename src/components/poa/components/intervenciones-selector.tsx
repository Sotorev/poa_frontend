...
export function IntervencionesSelectorComponent() {
  ...
  const [selectedIntervencionesIds, setSelectedIntervencionesIds] = useState<string[]>([])
  ...
  const handleSelectIntervencion = (intId: string) => {
    setSelectedIntervencionesIds(prev => {
      const newSelection = prev.includes(intId) 
        ? prev.filter(id => id !== intId) 
        : [...prev, intId]
      return newSelection.sort((a, b) => {
        const aInt = intervencionesList.find(int => int.id === a)
        const bInt = intervencionesList.find(int => int.id === b)
        if (!aInt || !bInt) return 0
        // Si ambas son personalizadas o ambas no lo son, ordena por número
        if (aInt.isCustom === bInt.isCustom) {
          return aInt.number - bInt.number
        }
        // Si solo una es personalizada, esa va después
        return aInt.isCustom ? 1 : -1
      })
    })
  }
  ...
  const handleAddNewIntervencion = () => {
    if (newIntervencion.trim() !== "") {
      const newInt: Intervencion = {
        id: `custom-${Date.now()}`,
        name: newIntervencion.trim(),
        number: customInterventionCounter,
        isCustom: true
      }
      setIntervencionesList(prev => [...prev, newInt])
      setSelectedIntervencionesIds(prev => {
        const newSelection = [...prev, newInt.id]
        return newSelection.sort((a, b) => {
          const aInt = intervencionesList.find(int => int.id === a) || newInt
          const bInt = intervencionesList.find(int => int.id === b) || newInt
          if (aInt.isCustom === bInt.isCustom) {
            return aInt.number - bInt.number
          }
          return aInt.isCustom ? 1 : -1
  ...
  const handleRemoveIntervencion = (intId: string, event: React.MouseEvent) => {
    ...
    setSelectedIntervencionesIds(prev => prev.filter(id => id !== intId))
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1">
        {selectedIntervencionesIds.map(id => {
              ...
              {filteredIntervenciones.map((int) => (
                ...
                >
                  <div className="flex items-center">
                    <Checkbox
                      checked={selectedIntervencionesIds.includes(int.id)}
