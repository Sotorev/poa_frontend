...
export function OdsField({ selected, onSelect, onRemove, error }: OdsFieldProps) {
					...
					<div>
						<div className="space-y-2">
							<div className="flex flex-wrap gap-2 mb-2">
								{selected.map((selectedOdsId) => {
									const ods = odsList.find((o) => o.odsId === selectedOdsId)
									if (!ods) return null
									return (
										<Badge
											key={ods.odsId}
											...
											{filteredODS.map((ods) => (
												...
												>
													<div className="flex items-center">
														<Checkbox
															checked={selected.includes(ods.odsId)}
															className="mr-2 h-4 w-4 rounded "
															style={{
																borderColor: `#${ods.colorHex}`,
																backgroundColor: selected.includes(ods.odsId)
																	? `#${ods.colorHex}`
																	: `#${ods.colorHex}33`,
																color: `ffffff`,
															}}
